...
highlight() { command highlight -s molokai "$@"; test $? = 0 -o $? = 141; }

case "$extension" in
    # Archive extensions:
    7z|a|ace|alz|arc|arj|bz|bz2|cab|cpio|deb|gz|jar|lha|lz|lzh|lzma|lzo|\
    rpm|rz|sdp|t7z|tar|tbz|tbz2|tgz|tlz|txz|tZ|tzo|war|xpi|xz|Z|zip)
    ...
    # PDF documents:
    pdf)
        try pdftotext -layout -l 10 -nopgbrk -q "$path" - && \
